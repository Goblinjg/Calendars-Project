...
export const TarefaController = {
  criar: async (req: Request, res: Response) => {
    ...
    const { descricao, data_entrega, user_id, materia_id, anotacao_id } = req.body;

    if (!descricao || !data_entrega || !user_id || !materia_id) {
      return res.status(400).json({ error: 'Descrição, Data e Matéria são obrigatórios.' });
    }
    ...
    if (entrega < hoje) {
       return res.status(400).json({ error: 'A data de entrega não pode ser no passado.' });
    }
  ...
  listar: async (req: Request, res: Response) => {
    ...
    if (!user_id) return res.status(400).json({ error: 'ID do usuário necessário.' });
